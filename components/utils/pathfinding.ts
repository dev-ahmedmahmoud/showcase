...
const OBSTACLES: ObstacleShape[] = [
  ...
  { type: 'box', center: new Vector3(-1.60, 0, -2.25), width: 0.2, depth: 4.5 },
  ...
  { type: 'box', center: new Vector3(-3.4, 0, -6.425), width: 4.5, depth: 3.35, rotation: Math.PI / 2 },
  
  // Gaming Chair - Position [-0.8, 0, -6.7], box footprint
  { type: 'box', center: new Vector3(-0.8, 0, -6.7), width: 1, depth: 1, rotation: Math.PI / 2 },
  
  // TV Stand - Position [2, 0, -0.3], scale 1.8, rotated 90°
  // Estimated dimensions: ~2.5m wide (rotated), ~0.6m deep
  { type: 'box', center: new Vector3(2, 0, -0.3), width: 0.6, depth: 2.5, rotation: Math.PI / 2 },
  
  // Couch - Position [2, 0, -3], scale 1.5, rotated -90°
  // Estimated base dimensions ~1.5m x ~0.6m, rotated -90° so width↔depth swap
  { type: 'box', center: new Vector3(2, 0, -3), width: 1.3, depth: 3, rotation: -Math.PI / 2 },
  ...
  { type: 'box', center: new Vector3(4, 0, -6.8), width: 3, depth: 2.6, rotation: -Math.PI },
...
export class SimplePath {
  ...
  private generateWaypointPath(start: Vector3, end: Vector3, waypoints: Vector3[]): PathPoint[] {
 ...
 */
function lineSegmentsIntersect(a1: Vector3, a2: Vector3, b1: Vector3, b2: Vector3, margin: number = 0.3): boolean {
  // Convert to 2D (using x and z coordinates)
  const ax1 = a1.x, az1 = a1.z, ax2 = a2.x, az2 = a2.z;
  const bx1 = b1.x, bz1 = b1.z, bx2 = b2.x, bz2 = b2.z;
  
  // Calculate distance from point to line segment
  const distanceToSegment = (px: number, pz: number, x1: number, z1: number, x2: number, z2: number): number => {
    const dx = x2 - x1;
    const dz = z2 - z1;
 ...
 */
function isPointInBox(point: Vector3, center: Vector3, width: number, depth: number, rotation: number = 0, margin: number = 0): boolean {
 ...
 */
function isPointInObstacle(point: Vector3, safetyMargin: number = SAFETY_MARGIN): boolean {
  for (const obstacle of OBSTACLES) {
    if (obstacle.type === 'circle') {
      ...
      if (isPointInBox(point, obstacle.center, obstacle.width, obstacle.depth, obstacle.rotation || 0, safetyMargin)) {
        return true;
      }
    }
  }
 ...
 */
function checkLineBoxCollision(start: Vector3, end: Vector3, center: Vector3, width: number, depth: number, rotation: number = 0, margin: number = SAFETY_MARGIN): boolean {
 ...
 */
function checkLineObstacleCollision(start: Vector3, end: Vector3): { hasCollision: boolean; obstacleIndex: number } {
  // First check walls
  ...
  for (let i = 0; i < OBSTACLES.length; i++) {
    ...
    if (obstacle.type === 'circle') {
      ...
      if (checkLineBoxCollision(start, end, obstacle.center, obstacle.width, obstacle.depth, obstacle.rotation || 0, SAFETY_MARGIN)) {
        return { hasCollision: true, obstacleIndex: i };
      }
    }
  }
 ...
 */
function generateAvoidanceWaypoints(start: Vector3, end: Vector3, obstacleIndex: number): Vector3[] {
  const candidates: Vector3[] = [];
  
  console.log(`  🎯 Generating waypoints for obstacle ${obstacleIndex}`);
  ...
  if (obstacleIndex === -2) {
    ...
    for (const wp of cornerWaypoints) {
      ...
      if (!toStart.hasCollision && !toEnd.hasCollision) {
        candidates.push(wp);
        console.log(`    ✅ Valid wall waypoint: [${wp.toArray()}]`);
      }
    }
    
    console.log(`  📍 Generated ${candidates.length} wall waypoint candidates`);
    return candidates;
  }
  
  // Regular obstacle collision - try both sides
  const obstacle = OBSTACLES[obstacleIndex];
  console.log(`  📦 Obstacle type: ${obstacle.type}, center: [${obstacle.center.toArray()}]`);
  ...
  for (const wp of strategicWaypoints) {
    if (!isPointInObstacle(wp, SAFETY_MARGIN)) {
      ...
      if (!toWp.hasCollision && !fromWp.hasCollision) {
        candidates.push(wp);
        console.log(`    ✅ Valid strategic waypoint: [${wp.toArray()}]`);
      } else {
        console.log(`    ❌ Strategic waypoint rejected: [${wp.toArray()}] (to:${toWp.hasCollision}, from:${fromWp.hasCollision})`);
      }
    } else {
      console.log(`    ❌ Strategic waypoint in collision zone: [${wp.toArray()}]`);
    }
  }
  ...
  for (const side of [1, -1]) {
    const waypoint = obstacle.center.clone().add(perpendicular.clone().multiplyScalar(side * avoidanceRadius));
    waypoint.y = 0;
    
    const validWaypoint = findClosestWalkablePoint(waypoint);
    
    console.log(`    Testing perpendicular waypoint (side ${side}): [${validWaypoint.toArray()}]`);
    
    // Verify waypoint itself is not in a collision zone
    if (isPointInObstacle(validWaypoint, SAFETY_MARGIN)) {
      console.log(`      ❌ Waypoint is in collision zone`);
    ...
    if (!toWaypoint.hasCollision && !fromWaypoint.hasCollision) {
      ...
      perpCount++;
      console.log(`      ✅ Valid perpendicular waypoint`);
    } else {
      console.log(`      ❌ Creates collision (to:${toWaypoint.hasCollision}, from:${fromWaypoint.hasCollision})`);
    }
  }
  ...
  if (obstacle.type === 'box') {
    ...
    for (const corner of corners) {
      ...
      console.log(`    Testing corner waypoint: [${validWaypoint.toArray()}]`);
      ...
      if (!toWaypoint.hasCollision && !fromWaypoint.hasCollision) {
        candidates.push(validWaypoint);
        cornerCount++;
        console.log(`      ✅ Valid corner waypoint`);
      } else {
        console.log(`      ❌ Creates collision (to:${toWaypoint.hasCollision}, from:${fromWaypoint.hasCollision})`);
      }
    }
  ...
  console.log(`  📍 Generated ${candidates.length} waypoint candidates (${perpCount} perpendicular, ${cornerCount} corners)`);
  return candidates;
}
...
export function isValidWalkablePosition(position: Vector3): boolean {
  ...
  if (x >= -3.5 && x <= -1.5) {
    ...
    if (x < -3.5 + margin || x > -1.5 - margin || z < -9 + margin || z > 0 - margin) {
      return false;
    }
  } 
  ...
  // So bounds: x from -1.5 to 5.3, z from -9 to 0
    ...
    if (x < -1.5 + margin || x > 5.3 - margin || z < -9 + margin || z > 0 - margin) {
      return false;
    }
  } else {
 ...
 */
function calculatePathDistance(from: Vector3, to: Vector3, waypoints: Vector3[]): number {
  let distance = 0;
  let current = from;
 ...
 */
function isPathCollisionFree(from: Vector3, to: Vector3, waypoints: Vector3[]): boolean {
  let current = from;
  let segmentIndex = 0;
  ...
  for (const wp of waypoints) {
    ...
    if (collision.hasCollision) {
      console.log(`  ❌ Segment ${segmentIndex} collision: [${current.toArray()}] → [${wp.toArray()}] hits obstacle ${collision.obstacleIndex}`);
      return false;
    }
    current = wp;
    segmentIndex++;
  ...
  if (finalCollision.hasCollision) {
    console.log(`  ❌ Final segment collision: [${current.toArray()}] → [${to.toArray()}] hits obstacle ${finalCollision.obstacleIndex}`);
    return false;
  }
 ...
 */
function findAllPaths(from: Vector3, to: Vector3, depth: number = 0, maxDepth: number = 5): Vector3[][] {
  ...
  let candidates = generateAvoidanceWaypoints(from, to, collision.obstacleIndex);
  
  // If no candidates found, try fallback waypoints in open areas
  if (candidates.length === 0 && depth < 2) {
    console.log(`  🔄 No direct waypoints, trying fallback waypoints`);
    candidates = getFallbackWaypoints(from, to);
  }
  
  if (candidates.length === 0) {
    console.log(`  ❌ No waypoint candidates found at depth ${depth}`);
    return []; // No valid waypoints found
  }
  
  console.log(`  🔁 Recursing with ${candidates.length} candidates at depth ${depth}`);
...
export function calculateSmoothPath(from: Vector3, to: Vector3): SimplePath {
  console.log('🔍 Calculating path from', from.toArray(), 'to', to.toArray());
  ...
  console.log('❌ Direct path blocked by obstacle', directCollision.obstacleIndex);
  ...
  if (allPaths.length === 0) {
    ...
    console.warn('⚠️ No valid path found, using direct path as fallback');
    return new SimplePath(from, to, []);
  }
  ...
  console.log(`✅ ${validPaths.length} collision-free paths, ❌ ${pathOptions.length - validPaths.length} paths with collisions`);
  
  if (validPaths.length === 0) {
    // All paths have collisions, fallback to shortest path anyway
    console.warn('⚠️ All paths have collisions, using shortest path anyway');
    console.warn('📍 Shortest path waypoints:', pathOptions[0].waypoints.map(w => w.toArray()));
    return new SimplePath(from, to, pathOptions[0].waypoints);
  }
  
  // Return the shortest collision-free path
  console.log('✅ Selected shortest collision-free path with', validPaths[0].waypoints.length, 'waypoints');
  console.log('📍 Waypoints:', validPaths[0].waypoints.map(w => w.toArray()));
